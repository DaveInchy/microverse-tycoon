import ambient from "mods@shaders/textures/moon_ambient.jpg";
import clouds from "mods@shaders/textures/clouds_atmosphere_colormap.jpg";
import color from "mods@shaders/textures/moon_colormap.jpg";
import depth from "mods@shaders/textures/moon_displacement.jpg";
import normal from "mods@shaders/textures/moon_normal.jpg";
import specular from "mods@shaders/textures/moon_specular.jpg";
    ...
    return (
        <group ref={planetRef}>
            ...
            {/* <mesh position={location}>
                <sphereGeometry
                    args={[5.05, 32, 16, Math.PI * (offSet * 1.5), Math.PI * 2, 0, Math.PI * 1]} />
                <meshPhongMaterial
                    map={T_CLOUDS}
                    opacity={0.2}
                    transparent={true}
            ...
            <mesh position={location}>
                <sphereGeometry
                    args={[5, 32, 16, Math.PI * offSet * 2, Math.PI * 2, 0, Math.PI * 1]}
                    onUpdate={geometry => {
                        geometry.center()
